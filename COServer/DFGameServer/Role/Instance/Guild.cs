...
namespace GameServer.Role.Instance
{
    public class Guild
    {
        ...
        {
            uint Money = EventsRewards.EventReward("CaptureTheFlagMoney", ConfigurableDropSystem.MoneyType.Money).RewardValue;

            uint[] wReward = new uint[2];
            if (CTF_Rank != 0)
            {
                if (CTF_Rank == 1)
                    wReward[0] = EventsRewards.EventReward("CaptureTheFlag").RewardValue;
                else if (CTF_Rank == 2)
                    wReward[0] = EventsRewards.EventReward("CaptureTheFlag2nd").RewardValue;
                else if (CTF_Rank == 3)
                    wReward[0] = EventsRewards.EventReward("CaptureTheFlag3rd").RewardValue;
                else if (CTF_Rank == 4)
                    wReward[0] = 100000;
                    ...
                    case (ushort)Role.Flags.GuildMemberRank.GuildLeader:
                        {
                            ...
                            if (Pool.GamePoll.TryGetValue(membru.UID, out client))
                            {
                                Online = true;
        ...
        {
            foreach (var user in Pool.GamePoll.Values.Where(e => e.Player.UID != sender))
            {
                if (user.Player.GuildID == Info.GuildID)
                    user.Send(packet);
            }
        ...
        public void Quit(string name, bool ReceiveKick, ServerSockets.Packet stream)
        {
            ...
            Client.GameClient client;
            if (Pool.GamePoll.TryGetValue(member.UID, out client))
            {
