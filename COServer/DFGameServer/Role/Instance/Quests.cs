...
namespace GameServer.Role.Instance
{
    ...
    public class Quests
    {
        ...
        {
            Game.MsgServer.MsgQuestList.QuestListItem item;
            if (src.TryGetValue(UID, out item))
            {
        ...
        {
            if (global::Core.Features.FeatureRegistry.IsBlockedQuest(quest.MissionId)) { _quest = null; return false; } // [feature-gate quests]
            _quest = null;
        ...
        {
            if (global::Core.Features.FeatureRegistry.IsBlockedQuest(quest.MissionId)) return false; // [feature-gate quests]
            if(quest.MissionId == 1)
            {
                if (Player.QuestEntries >= Pool.NewLottery.QuestEntry(Player.VipLevel))
                {
                    return false;
                }
            }
            if (quest.MissionId == 4)
            {
                if (Player.Quest2Entries >= Pool.NewLottery.Quest2Entry(Player.VipLevel))
                {
                    return false;
                }
            }
            if (quest.MissionId == 5)
            {
                if (Player.Quest3Entries >= Pool.NewLottery.Quest2Entry(Player.VipLevel))
                {
                    return false;
                }
            }
