...
namespace GameServer.Role.Instance
{
    ...
    public class Chi : IEnumerable<Chi.ChiPower>
    {
        ...
        {
            public static readonly RandomLite Rand = new RandomLite();
            ...
            public static Counter EpicAtribut = new Counter(0);
            public static int StatsRand(ChiAttributeType typ, Game.MsgServer.MsgChiInfo.ChiPowerType StageType, string Name, bool epic = false, bool GMCommand = false)
            {
                ...
                if ((byte)typ < 6)
                {
                    ...
                    ushort val = (ushort)Rand.Next(194, 200);

                    val += 30;
                    if (val > 200)
                        val -= 30;
                    ushort val1 = (ushort)Rand.Next(140, val);
                    uint ID = (uint)data.Item1;
                    if (EpicAtribut.Now == 2000001)
                    {
                        val1 = 200;
                        EpicAtribut.Set(0);
                    }
                ...
                else
                {
                    ...
                    ushort val = (ushort)Rand.Next(80, 100);

                    ushort val1 = 0;
                    if (val > 60)
                        val1 = (ushort)Rand.Next(80, val);
        ...
        public static int MedPower(ChiAttributeType power)
        {
            if (power == ChiAttributeType.CriticalStrike)
                return 185;
            else if (power == ChiAttributeType.SkillCriticalStrike)
                ...
                return 250;
            else if (power == ChiAttributeType.FinalMagicDefense)
                return 250;
        ...
        {
            if (!ServerConfig.EnabledChi) return; // [feature-gate progression.chi]
            lock (SynRoot)
            {
