...
namespace GameServer.ServerSockets
{
    public class ServerSocket
    {
        ...
        private bool DisableBruteforceCheck = true;
        ...
        public Action<SecuritySocket> ProcessDisconnect;
            ...
            try
            {
                if (Alive)
                {
                    if (Connection.Poll(0, SelectMode.SelectRead))
                    {
                        ...
                        if (!Bruteforce.IsBanned(RemoteIP) || Bruteforce.AllowAddress(RemoteIP) || RuntimeInformation.IsOSPlatform(OSPlatform.Linux) || DisableBruteforceCheck) // In linux bypass security
                        {
                        ...
                        {
                            Console.WriteLine($"Connection closed by security. [{RemoteIP}]");
                            CloseNewSocket(socket);
                            Console.WriteLine($"Allowed IP: {Bruteforce.AllowAddress(RemoteIP)}");
                            Console.WriteLine($"IsBanned: {Bruteforce.IsBanned(RemoteIP)}");
                        }
                    }
                }
            }
