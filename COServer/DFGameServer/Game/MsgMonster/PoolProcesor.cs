...
namespace GameServer.Game.MsgMonster
{
    public class PoolProcesses
    {
                ...
                foreach (var map_mob in Array)
                {
                    ...
                    if ((Guard.Family.Settings & MonsterSettings.Guard) == MonsterSettings.Guard)
                    {
                        if (timer > Guard.AttackSpeed.AddMilliseconds(Guard.Family.AttackSpeed + 1000))
                        {
                            ...
                            if (!Guard.Alive)
                            {
                                Guard.AddFadeAway(timer.Ticks, client.Map);
                                Guard.RemoveView(timer.Ticks, client.Map);
        ...
        public static void AliveMonstersCallback(Client.GameClient client)
        {
                ...
                foreach (var map_mob in Array)
                {
                        ...
                        && (monster.Family.Settings & MonsterSettings.Lottus) != MonsterSettings.Lottus)
                    {
                        ...
                        if (!Mob.Alive)
                        {
                            ...
                            Mob.AddFadeAway(now.Ticks, client.Map);
                            Mob.RemoveView(now.Ticks, client.Map);
                        }
                    }
                }
            ...
            {
                //stopwatch.Stop();
                //Console.WriteLine($"AliveMonstersCallback ejecutado en {stopwatch.Elapsed.TotalMilliseconds} ms (Player: {client.Player.Name})");
            }
