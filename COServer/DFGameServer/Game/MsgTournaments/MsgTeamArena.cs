...
namespace GameServer.Game.MsgTournaments
{
    public class MsgTeamArena
    {
        ...
        {
            if (!Core.Features.FeatureRegistry.IsKept("events.teamarena")) return; // [feature-gate]
                    ...
                    using (var rec = new ServerSockets.RecycledPacket())
                    {
                        ...
                        foreach (var user in winner.GetMembers())
                        {
                            //if (user.Player.DynamicID == dinamicID)
                            {
                                ...
                                {
                                    user.HonorPoints += 400;
                                    user.SendSysMesage("You received 400 HonorPoints.", MsgMessage.ChatMode.System);
                                    if (user.Inventory.HaveSpace(1))
                                        user.Inventory.AddItemWitchStack(723912, 0, 1, stream);
                                    else
                                        user.Inventory.AddReturnedItem(stream, 723912);
                                }
                                //if (user.TeamArenaStatistic.Info.TodayBattles == 20)
                                //{
                                //    user.HonorPoints += 300;
                                //    user.SendSysMesage("You received 300 HonorPoints.", MsgMessage.ChatMode.System);
                                //    if (user.Inventory.HaveSpace(1))
                                //        user.Inventory.AddItemWitchStack(723912, 0, 1, stream);
                                //    else
                                //        user.Inventory.AddReturnedItem(stream, 723912);
