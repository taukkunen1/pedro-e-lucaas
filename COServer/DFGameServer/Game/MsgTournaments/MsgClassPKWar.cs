...
namespace GameServer.Game.MsgTournaments
{
    public class MsgClassPKWar
    {
        ...
        public const string FileName = "ClassPkWar.ini";
        ...
        public MsgClassPKWar(ProcesType _Proces)
        {
            ...
            DataManager = new ClassPKWarManager();
        }
        internal void Start()
        {
        ...
        internal ProcesType GetWar(Client.GameClient client, out War mywar)
        {
            ...
            {
                Console.WriteLine($"ClassPkWar > Class : {client.Player.Class} || Lev : {client.Player.Level} ?? not found!");
                mywar = null;
                return ProcesType.Dead;
            }
        }
        ...
        public class War
        {
            ...
            {
                if (!Core.Features.FeatureRegistry.IsKept("events.classpkwar")) return; // [feature-gate]
                if (Proces == ProcesType.Dead)
                {
                    Proces = ProcesType.Alive;
                    FinishTimer = DateTime.Now.AddMinutes(30);
            ...
            public void GetMyReward(Client.GameClient client, ServerSockets.Packet stream)
            {
                if (IsFinish(client))
                {
                    ...
                    client.SendSysMesage("You received " + value.ToString() + " ConquerPoint. ", MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.red);
                    string reward = "[EVENT]" + client.Player.Name + " has received " + value + "  ConquerPoints from class pk.";
                    Database.ServerDatabase.LoginQueue.Enqueue(reward);
                    LastFlag = aura;
                    Winner = client.Player.UID;
                    MsgSchedules.SendSysMesage("" + client.Player.Name + " Won " + Typ.ToString() + " PK War (" + Level.ToString() + ") , he received Top " + Typ.ToString() + ", " + value.ToString() + " ConquerPoints !", MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.white);
