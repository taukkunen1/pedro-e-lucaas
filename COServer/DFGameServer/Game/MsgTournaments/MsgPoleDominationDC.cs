...
namespace GameServer.Game.MsgTournaments
{
    ...
    public class MsgPoleDominationDC
    {
        ...
        public class GuildWarScore
        {
            ...
            {
                ConquerPointsReward = EventsRewards.EventReward("MsgPoleDominationDC").RewardValue;
            }
        }
        ...
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Pool.ServerMaps[1000].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 832));
        }
        ...
        internal unsafe void Start()
        {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                ScoreList.Clear();
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("PoleDomination of DC war in ApeCity(Left Gate) has started!", MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.System).GetArray(stream));
            }
        }
        ...
        internal unsafe void Began()
        {
            if (Proces == ProcesType.Idle)
            {
                ...
                {
                    var stream = rec.GetStream();
                    Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("PoleDomination of DC has began!", MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.System).GetArray(stream));
                }
            }
        }
