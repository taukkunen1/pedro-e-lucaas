...
namespace GameServer.Game.MsgTournaments
{
    public class MsgArena
    {
        ...
        public List<uint> BlockArenaMaps = new List<uint>() { 1858, 1860, 4020, 1111, 1112, 1113, 1114, 4000, 5263,1767, 4003, 4006, 4008, 4009, 6000, 6001, 1017, 1080, 1081, 2060, 6002, 6003, 601, 700, 1038, MsgClassPKWar.MapID, 1036, 1764, Game.MsgTournaments.MsgEliteGroup.WaitingAreaID };
        ...
        public void CheckGroups(DateTime clock)
        {
        ...
        public void CreateMatches(DateTime clock)
        {
                ...
                if (Timer.Second % 3 == 0)
                {
                    ...
                    foreach (var user in Players)
                    {
                        if (BlockArenaMaps.Contains(user.Player.Map) ||
                            !user.Player.Alive || 
                            user.InQualifier() || user.Socket.Alive == false
                            ...
                            && user.ArenaStatistic.Info.Status == MsgServer.MsgArenaInfo.Action.WaitingForOpponent)
                        {
                            if (user1 == null)
                                user1 = user;
                            else if (user2 == null && user1.Player.UID != user.Player.UID)
                                user2 = user;
        ...
        public void VerifyMatches(DateTime clock)
        {
        ...
        public static void UpdateRank()
        {
            lock (Top1000Today)
            {
                ...
                var array = ArenaPoll.Values.Where(x => BaseFunc.UserExists(x.UID) && BaseFunc.UserIsNormal(x.Name)).ToArray();
            ...
            public void Win(Client.GameClient winner, Client.GameClient loser)
            {
                ...
                if (winner.ArenaMatch != null && loser.ArenaMatch != null)
                {
                    using (var rec = new ServerSockets.RecycledPacket())
                    {
                        ...
                        {
                            winner.HonorPoints += 1500;
                            winner.SendSysMesage("You received 1500 HonorPoints.", MsgMessage.ChatMode.System);
                            if (winner.Inventory.HaveSpace(1))
                                winner.Inventory.AddItemWitchStack(723912, 0, 1, stream, true);
                            else
                        ...
                        {
                            winner.HonorPoints += 2000;
                            winner.SendSysMesage("You received 2000 HonorPoints.", MsgMessage.ChatMode.System);
                            if (winner.Inventory.HaveSpace(1))
                                winner.Inventory.AddItemWitchStack(723912, 0, 1, stream, true);
                            else
                                winner.Inventory.AddReturnedItem(stream, 723912);
                        ...
                        {
                            winner.HonorPoints += 1200;
                            loser.SendSysMesage("You received 1200 HonorPoints.", MsgMessage.ChatMode.System);
                            if (loser.Inventory.HaveSpace(1))
                                loser.Inventory.AddItemWitchStack(723912, 0, 1, stream, true);
                            else
        ...
        public void DoSignup(ServerSockets.Packet stream, Client.GameClient client)
        {
            ...
            {
                client.SendSysMesage("Only level 70+ can sign up for arena.", MsgServer.MsgMessage.ChatMode.Agate, MsgServer.MsgMessage.MsgColor.red);
                return;
            }
            ...
            if (BlockArenaMaps.Contains(client.Player.Map))
            {
                ...
                client.SendSysMesage("You cant Join because you are in Event!.", MsgServer.MsgMessage.ChatMode.Agate, MsgServer.MsgMessage.MsgColor.red);
                return;
            }
