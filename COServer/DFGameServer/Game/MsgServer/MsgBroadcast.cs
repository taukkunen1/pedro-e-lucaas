                    ...
                    case BroadTyp.BroadcastMessage:
                        {
                            ...
                            if (user.Player.ConquerPoints >= 5)
                            {
                                ...
                                broadcast.Message = MsgFilter.Filter(broadcast.Message, user);
                                #region AntiSpam
                                foreach (var bad in MsgFilter.Insults)
                                {
                                    if (broadcast.Message.StartsWith(bad))
                                    {
                                        broadcast.Message = broadcast.Message.Replace(bad, "******");
                                        user.Socket.Disconnect();
                                    }
                                    if (broadcast.Message.Contains(" " + bad))
                                    {
                                        broadcast.Message = broadcast.Message.Replace(" " + bad, "******");
                                        user.Socket.Disconnect();
                                    }
