         ...
         stream.GetTradePartner(out UID, out Mode, out Online, out HoursLeft);
            ...
            switch (Mode)
            {
                case Action.RequestPartnership:
                    {
                        if (user.Player.Associate.AllowAdd(Role.Instance.AssociateGS.Partener,UID, 10))
                        {
                            Role.IMapObj obj;
                            if (user.Player.View.TryGetValue(UID, out obj, Role.MapObjectType.Player))
                            {
                                ...
                                {
                                    if (user.Player.Associate.AllowAdd(Role.Instance.AssociateGS.Partener, player.UID, 10))
                                    {
                                        if (player.Associate.AllowAdd(Role.Instance.AssociateGS.Partener, user.Player.UID, 10))
                                        {
                ...
                case Action.BreakPartnership:
                    {
                        if (user.Player.Associate.Remove(Role.Instance.AssociateGS.Partener, UID))
                        {
                            ...
                            {
                                if (target.Player.Associate.Remove(Role.Instance.AssociateGS.Partener, user.Player.UID))
                                {
                                    target.Send(stream.TradePartnerCreate(user.Player.UID, Action.BreakPartnership, Online, HoursLeft, ""));
                                }
                            }
                            else
                                Role.Instance.AssociateGS.RemoveOffline(Role.Instance.AssociateGS.Partener, UID, user.Player.UID);
                        }
                        break;
                    }
