                        ...
                        if (user.Player.MyMentor != null)
                        {
                            if (Pool.GamePoll.ContainsKey(user.Player.MyMentor.MyUID))
                            {
                                ...
                                if (user.Player.MyMentor.OnlineApprentice.TryRemove(user.Player.UID, out remover))
                                {
                                    ...
                                    user.Player.Associate.Remove(Role.Instance.AssociateGS.Mentor, user.Player.MyMentor.MyUID);
                                    user.Player.MyMentor.Remove(Role.Instance.AssociateGS.Apprentice, user.Player.UID);
                            ...
                            else
                            {
                                ...
                                Role.Instance.AssociateGS.RemoveOffline(Role.Instance.AssociateGS.Apprentice, user.Player.MyMentor.MyUID, user.Player.UID);
                                user.Player.Associate.Remove(Role.Instance.AssociateGS.Mentor, user.Player.MyMentor.MyUID);
                                user.Player.MyMentor = null;
                            }
                        }
                ...
                case Action.ExpellApprentice:
                    {
                        ...
                        if (Pool.GamePoll.TryGetValue(dwParam, out pClient))
                        {
                            ...
                            if (pClient.Player.MyMentor != null)
                            {
                                ...
                                user.Player.Associate.Remove(Role.Instance.AssociateGS.Apprentice, pClient.Player.UID);
                                pClient.Player.Associate.Remove(Role.Instance.AssociateGS.Mentor, user.Player.UID);
                            }
                            user.Player.Associate.Remove(Role.Instance.AssociateGS.Apprentice, dwParam);
                            pClient.Player.MyMentor = null;
                        }
                        else
                        {

                            Role.Instance.AssociateGS.RemoveOffline(Role.Instance.AssociateGS.Mentor, dwParam, user.Player.UID);
                            user.Player.Associate.Remove(Role.Instance.AssociateGS.Apprentice, dwParam);
                        }
                ...
                case Action.AcceptRequestApprentice:
                    {
                        ...
                        if (user.Player.View.TryGetValue(UID, out obj, Role.MapObjectType.Player))
                        {
                            ...
                            {
                                if (target.Associate.AllowAdd(Role.Instance.AssociateGS.Apprentice, user.Player.UID, 10))
                                {
                                    if (user.Player.Associate.AllowAdd(Role.Instance.AssociateGS.Mentor, target.UID, 1))
                                    {
                ...
                case Action.AcceptRequestMentor:
                    {
                        ...
                        if (user.Player.View.TryGetValue(UID, out obj, Role.MapObjectType.Player))
                        {
                            ...
                            if (ShareBattlePower == 1)
                            {
                                if (target.Associate.AllowAdd(Role.Instance.AssociateGS.Mentor, user.Player.UID, 1))
                                {
                                    if (user.Player.Associate.AllowAdd(Role.Instance.AssociateGS.Apprentice, target.UID, 10))
                                    {
