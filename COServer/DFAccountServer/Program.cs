...
namespace AccountServer
{
    class Program
    {
            ...
            while (player.Queue.CanDequeue())
            {
                ...
                if (id == 1124 && len == 240)
                {
                    ...
                    if (Core.Models.Server.Servers.TryGetValue(ServerSelected, out ServerInfo Server))
                    {
                        ...
                        if (player.Account.Exists() && VerifyLogin(player.Info.Username, player.Info.Password))
                        {
        ...
        /// <summary>A API confere a senha (hash); o AccountServer nunca recebe a senha guardada.</summary>
        private static bool VerifyLogin(string username, string password)
        {
            Account logged = RestApiHelper.PostRequestAs<Account>("AccountLogin", new { Username = username, Password = password });
            return logged != null && logged.EntityID != 0;
        }
